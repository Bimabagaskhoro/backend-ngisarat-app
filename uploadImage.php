<?php
    ...
    if($method == 'POST'){
        if(isset($_POST['tittle_alphabet']) AND isset($_POST['description_alphabet'])) {
            ...
            $sql = "INSERT INTO databisindo (tittle_alphabet, description_alphabet, image_alphabet) VALUES ('$tittle_alphabet', '$description_alphabet', '$image_name')" ;

            $connect -> query($sql);

            $result['data'] = [
                "tittle" => $tittle_alphabet,
                "description" => $description_alphabet,
                "image" => $image_name
            ];
        } else {
            $result['status'] = [
